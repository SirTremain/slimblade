...
void appm_update_phy_param(void)
{
    #if(ENABLE_PHY_2M) 
    uart_printf("%s \r\n",__func__);
    // Prepare the GAPC_PARAM_UPDATE_CMD message
    ...
    cmd->tx_phy = GAP_PHY_LE_2MBPS;//conn_param->tx_phy; //GAP_PHY_LE_2MBPS;
    /// Supported LE PHY for data reception (@see enum gap_phy)
    cmd->rx_phy = GAP_PHY_LE_2MBPS;//conn_param->rx_phy; //GAP_PHY_LE_2MBPS;
    /// PHY options (@see enum gapc_phy_option)
    cmd->phy_opt = GAPC_PHY_OPT_LE_CODED_ALL_RATES;//conn_param->phy_opt;
