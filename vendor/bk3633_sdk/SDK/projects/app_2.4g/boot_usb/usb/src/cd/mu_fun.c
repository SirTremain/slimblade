				    ...
				    MUSB_FunctionClient* pClient)
{
    ...
    MUSB_DPRINTF("bnumconfiguration = 0x%x, sizeof(MUSB_ConfigurationDescriptor) = 0x%x\r\n",
...
uint8_t MGC_FunctionParseSetup(MGC_Port* pPort, uint8_t* pbStatus)
{
	...
	//uart_printf("MGC_FunctionParseSetup tst\r\n");
    /* seed success */
    *pbStatus = MUSB_STATUS_OK;
    ...
    //bim1_uart_printf("MGC_FunctionParseSetup: pRequest = 0x%p\r\n", pRequest);
	
    //bim1_uart_printf("musb type = 0x%p\r\n", pRequest->bmRequestType & MUSB_TYPE_MASK);
    ...
    if (pRequest && (MUSB_TYPE_STANDARD == (pRequest->bmRequestType & MUSB_TYPE_MASK)))
    {
	 ...
	 //bim1_uart_printf("MGC_FunctionParseSetup: pRequest->bmRequestType = 0x%x, bRecip = 0x%x, wValue = 0x%x, wIndex = 0x%x\r\n",
    //            pRequest->bmRequestType, bRecip, wValue, wIndex);
	 
	 //bim1_uart_printf("pRequest->bRequest = 0x%x\r\n",pRequest->bRequest);
    ...
    //        GPIO_Output(26, FALSE);
        ...
        case MUSB_REQ_GET_DESCRIPTOR:
            ...
            MUSB_DPRINTF("GET_DESCRIPTOR: bRecip=%d, wValue=0x%x\r\n", bRecip, wValue);
		//bim1_uart_printf("GET_DESCRIPTOR: bRecip=%d, wValue=0x%x,w_index=%x\r\n", bRecip, wValue,wIndex);
            if (MUSB_RECIP_DEVICE == bRecip)
            {
                bOurs = TRUE;
		   ...
		   //bim1_uart_printf("RECIP_INTERFACE\r\n");
                bOurs = TRUE;
                wLength = MUSB_SWAP16P((uint8_t*)&(pRequest->wLength));
                bOK = FALSE;
                bType = bQueryType = (uint8_t)(wValue >> 8);
