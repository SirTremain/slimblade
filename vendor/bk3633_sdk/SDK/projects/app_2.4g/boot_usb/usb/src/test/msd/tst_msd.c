...
int usb_sw_init(void)
{
    ...
    {
		bim_printf("error ,could not initialize MicroSW\r\n");
		return -1;
    }

    /* find first CDI port */
    MGC_pCdiPort = MUSB_GetPort(0);
	bim_printf("usb_sw_init: phase 2\r\n");
    /* start session */
    ...
    if(!MGC_hCdiBus)
    {
        bim_printf("usb_sw_init: MUSB_RegisterOtgClient fail\r\n");
				
        return -1;
    }
    else
    {
        bim_printf("usb_sw_init: MGC_hCdiBus\r\n");
    }
    return 0;
}
