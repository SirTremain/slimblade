...
//---------------------------------------------------------------------------------------------------// 
		...
		//RAMCTRL(default)
		WriteComm(0xD7);  	
		WriteData(0x20); //0x20  2data    默认0x00
		WriteData(0x00);  
...
{
		int tempdata;
		unsigned char colordata[64];
		unsigned char colordata1[64];
		unsigned char colordata2[64];
		for(tempdata=0;tempdata<64;tempdata++)
		{
			colordata[tempdata]=color;
		}
		for(tempdata=0;tempdata<64;tempdata++)
		{
			colordata1[tempdata]=color-1;
		}
		for(tempdata=0;tempdata<64;tempdata++)
		{
			colordata2[tempdata]=color-2;
		}
		WriteComm(0x2A);     //Column Address Set
		WriteData(0x00);   
		WriteData(0x00);   //0
		...
		for(tempdata=0;tempdata<18;tempdata++)
		{
			spi_dma_write(framebuffer,1024,NULL);
		}
...
void DispPicFromSD(unsigned char PicNum)
{
	...
	k=(unsigned long)1080*(unsigned long)360*2/512;	 
...
void DispScaleHor1(void)
{
	...
	for(k=0;k<ROW/4;k++)
	{
		for(i=0;i<COL%32;i++){DBL=0x00;WR0=0;WR0=1;DBL=0x00;WR0=0;WR0=1;}
		for(i=0;i<32;i++)
		{
			for(j=0;j<COL/32;j++)	 
			{
...
void DispScaleHor2(void)
{
	...
	for(k=0;k<ROW/6;k++)
	{
		for(i=0;i<COL%64;i++){DBL=0x00;WR0=0;WR0=1;DBL=0x00,WR0=0;WR0=1;}
		for(i=0;i<64;i++)
		{
			for(j=0;j<COL/64;j++)
			{
...
{  
unsigned int ReadValue=0x00;
WriteComm(0xD3);
Read_Data();                    //Dummy read
    Read_Data();                    //Dummy read
     Read_Data();                    //0X93
     Read_Data();               //Dummy read
ReadValue=Read_Data();    //ReadValue=0x65 for HX8352B
return ReadValue;
...
void DispRegValue(unsigned int RegIndex,unsigned char ParNum)
{
    ...
    for(i=0;i<ParNum;i++)
	{
		...
		DispInt(d,FONT_W*2,(FONT_H+1)*(i+1),BLUE,WHITE);
	}
