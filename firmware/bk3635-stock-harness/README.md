...
This isolated workspace builds five reviewed injections while retaining complete
...
`experiment-entry-probe`, `rust-response-probe`, or `post-init-hook-probe`
command from the repository root. Generated `DO_NOT_FLASH` artifacts remain
under `target/`.
